      ...
      <div className="min-h-screen bg-gray-100 dark:bg-gray-900 py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div className="flex gap-2 sm:gap-4 w-full sm:w-auto flex-col sm:flex-row">
               <button onClick={() => setIsReportModalOpen(true)} className="px-2 sm:px-6 py-1.5 sm:py-3 text-xs sm:text-base bg-green-600 text-white rounded-lg hover:bg-green-700 whitespace-nowrap font-medium">📊 Generate Report</button>
              <button onClick={() => router.push('/invoices/create')} className="px-2 sm:px-6 py-1.5 sm:py-3 text-xs sm:text-base bg-blue-600 text-white rounded-lg hover:bg-blue-700 whitespace-nowrap font-medium">+ Create New Invoice</button>
            </div>
          </div>
          ...
          {loading ? (
            ...
            //invoices/page.tsx - Replace the table section with this:
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className="bg-gray-50 dark:bg-gray-700/50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase whitespace-nowrap">Invoice #</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase whitespace-nowrap">Biller Name</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase whitespace-nowrap">Date</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase whitespace-nowrap">Amount</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase whitespace-nowrap">E-Way Bill No.</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase whitespace-nowrap">Actions</th>
                    </tr>
                  </thead>
                    ...
                    {filteredInvoices.length === 0 ? (
                      ...
                      filteredInvoices.map((invoice) => (
                        <tr key={invoice.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                          <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-white whitespace-nowrap">{invoice.invoiceNumber}</td>
                          <td className="px-6 py-4 text-sm text-gray-900 dark:text-white whitespace-nowrap">{invoice.customerName}</td>
                          <td className="px-6 py-4 text-sm text-gray-900 dark:text-white whitespace-nowrap">{new Date(invoice.invoiceDate).toLocaleDateString('en-IN')}</td>
                          <td className="px-6 py-4 text-sm text-gray-900 dark:text-white whitespace-nowrap">₹{invoice.totalAmount.toFixed(2)}</td>
