            ...
            {selectedCustomer && (
              <div className="p-4 bg-blue-50 dark:bg-gray-800/50 rounded-lg">
                 ...
                 <div className="mt-4 space-y-3">
                   <div>
                     <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Invoice Copies Required</label>
                     <div className="space-y-2">
                       <label className="flex items-center">
                         <input
                           type="checkbox"
                           checked={invoiceCopies.includes('original')}
                           onChange={(e) => {
                             if (e.target.checked) {
                               setInvoiceCopies([...invoiceCopies, 'original']);
                             } else {
                               setInvoiceCopies(invoiceCopies.filter(c => c !== 'original'));
                             }
                           }}
                           className="mr-3"
                         />
                         <span className="text-sm text-gray-700 dark:text-gray-300">Original for Recipient</span>
                       </label>
                       <label className="flex items-center">
                         <input
                           type="checkbox"
                           checked={invoiceCopies.includes('duplicate1')}
                           onChange={(e) => {
                             if (e.target.checked) {
                               setInvoiceCopies([...invoiceCopies, 'duplicate1']);
                             } else {
                               setInvoiceCopies(invoiceCopies.filter(c => c !== 'duplicate1'));
                             }
                           }}
                           className="mr-3"
                         />
                         <span className="text-sm text-gray-700 dark:text-gray-300">Duplicate for Transporter (Copy 1)</span>
                       </label>
                       <label className="flex items-center">
                         <input
                           type="checkbox"
                           checked={invoiceCopies.includes('duplicate2')}
                           onChange={(e) => {
                             if (e.target.checked) {
                               setInvoiceCopies([...invoiceCopies, 'duplicate2']);
                             } else {
                               setInvoiceCopies(invoiceCopies.filter(c => c !== 'duplicate2'));
                             }
                           }}
                           className="mr-3"
                         />
                         <span className="text-sm text-gray-700 dark:text-gray-300">Duplicate for Transporter (Copy 2)</span>
                       </label>
                       <label className="flex items-center">
                         <input
                           type="checkbox"
                           checked={invoiceCopies.includes('triplicate')}
                           onChange={(e) => {
                             if (e.target.checked) {
                               setInvoiceCopies([...invoiceCopies, 'triplicate']);
                             } else {
                               setInvoiceCopies(invoiceCopies.filter(c => c !== 'triplicate'));
                             }
                           }}
                           className="mr-3"
                         />
                         <span className="text-sm text-gray-700 dark:text-gray-300">Triplicate for Supplier</span>
                       </label>
                     </div>
                   </div>
                   <div>
